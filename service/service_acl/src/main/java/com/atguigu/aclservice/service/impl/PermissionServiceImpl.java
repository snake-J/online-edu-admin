...
//        });
    ...
    @Override
    public List<Permission> queryAllMenu2() {
        QueryWrapper<Permission> queryWrapper=new QueryWrapper<>();
        queryWrapper.orderByDesc("id");
        List<Permission> permissionList = baseMapper.selectList(queryWrapper);
        List<Permission> resultList=bulidPermission2(permissionList);
        return resultList;
    }


    public static List<Permission> bulidPermission2(List<Permission> permissionList) {
        List<Permission> finalNode=new ArrayList<>();
        for (Permission permissionNode : permissionList) {
            if("0".equals(permissionNode.getPid())){
                permissionNode.setLevel(1);
    ...
    private static Permission selectChildren2(Permission permissionNode, List<Permission> permissionList) {
        permissionNode.setChildren(new ArrayList<Permission>());
        for (Permission it : permissionList) {
            if (it.getPid().equals(permissionNode.getId())) {
                ...
                permissionNode.getChildren().add(selectChildren2(it, permissionList));
            }
        }
        return permissionNode;
    }
    //===================================递归删除菜单
    @Override
    public void removeChildById2(String id) {
        //1.创建list集合，用于封装该id号下的所有子菜单id
        List<String> idList=new ArrayList<>();
        //2.向list集合添加所有要删除的菜单id
        this.selectIds(id,idList);
        //3.当前的id封装进集合，调用方法进行删除
        idList.add(id);
        baseMapper.deleteBatchIds(idList);
    }
    ...
    //根据当前菜单id，查询菜单下的所有子菜单，封装到list集合中
...
//            this.selectIds(permission.getId(),idList);
//        }
        permissions.stream().forEach(item->{
            idList.add(item.getId());
            this.selectIds(item.getId(),idList);
        });
    }
    //==================================给角色分配菜单
    @Override
    public void saveRolePermissionRealtionShipGuli2(String roleId, String[] permissionId) {
